 ...
 * @date     $Date: 2006/06/06 14:10:02 $
 */
public class ErdSaveDialog extends JDialog 
    ...
    private String save(String path, int fileFormat) {
        ...
        try {
            ...
            if (fileFormat == PNG_FORMAT) {
                ImageHandlerPNGEncoder ihandler = new ImageHandlerPNGEncoder(
                                                        file.getParent(), null);
                ihandler.encodeImage(buffImage, file);
            }
            else if (fileFormat == SVG_FORMAT)  {
                ...
                g = new SVGGraphics2D(ctx, svgFontCheckbox.isSelected());
                g.drawImage(buffImage, new AffineTransform(1f,0f,0f,1f,0,0), null);
                ...
                out.close();               
                    ...
                    // ---------------------------------------------
                    // for JPEG images using javax.imageio package
                    // also works well - no quality setting however.
                    // used as shown below:
                    // ---------------------------------------------
                    ...
                    JPEGEncodeParam jpegParam = jpegEncoder.getDefaultJPEGEncodeParam(buffImage);

                    float quality = (qualitySlider.getValue()*10)/100.0f;
                    jpegParam.setQuality(quality, false);
                    jpegEncoder.setJPEGEncodeParam(jpegParam);
                    jpegEncoder.encode(buffImage);
                    bfos.flush();
                    bfos.close();
            }
            else if (fileFormat == GIF_FORMAT) {
                fos = new FileOutputStream(file);
                GifEncoder gifEncoder = new GifEncoder(buffImage, fos);
                gifEncoder.encode();
            }
    ...
    private String saveApplicationFileFormat(File file) {
        ...
        SystemUtilities.addRecentOpenFile(file.getAbsolutePath());
        GUIUtilities.scheduleGC();
