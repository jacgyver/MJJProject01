...
//import org.executequery.util.IconUtilities;

/*
 * @(#)CloseTabbedPaneUI.java	1.126 03/01/23
 *
    ...
    */
    ...
    public class Handler implements MouseListener, 
        ...
        public void mouseReleased(MouseEvent e) {
            ...
            // check if we have a popup menu available
            if (e.isPopupTrigger()) {
                if (tabPane instanceof CloseTabbedPane) {
                    CloseTabbedPane closeTabPane = (CloseTabbedPane)tabPane;
                    if (closeTabPane.isTabPopupEnabled()) {
                        
                        for (int i = 0, k = tabPane.getTabCount(); i < k; i++) {
                            Rectangle tabRect = rects[i];
                            if (tabRect.contains(x, y)) {
                                closeTabPane.showPopup(i, x, y);
                                break;
                            }
                        }
